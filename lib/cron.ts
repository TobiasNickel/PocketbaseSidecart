...
export async function runCron(pb: PocketBase){
    ...
    cronList.forEach((record) => {
        const job = createCronjob(record, cronCollection)
        job.start()
    ...
    const unsubscribe = await cronCollection.subscribe('*', async (event) => {
        if (event.action === 'create') {
            ...
            const newJob = createCronjob(event.record, cronCollection)
            newJob.start()
...
function createCronjob(record: RecordModel, cronCollection: RecordService<RecordModel>) {
